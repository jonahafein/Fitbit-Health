...
class Fitbit_App():
    ...
    def callback(self, code:str):
        ...
        self.user_tokens['access_token'] = tokens['access_token']
        self.user_tokens['user_id'] = tokens['user_id']
    ...
    def dashboard(self,request: Request):
        ...
        # heart rate data 
        heart_url = f'https://api.fitbit.com/1/user/-/activities/heart/date/2025-10-30/{self.today}/1min.json'
        ...
        url = f"https://api.fitbit.com/1/user/-/hrv/date/2025-10-30/{self.today}.json"
        ...
        # sleep data
        sleep_url = f'https://api.fitbit.com/1.2/user/-/sleep/date/2025-10-29/{self.today}.json'
    ...
    def run(self):
        self.config = Config(app = self.app, host="127.0.0.1", port=8000, log_level = "info")
        self.server = Server(config = self.config)
        with self.server.run_in_thread(): # run and automatically stop server
